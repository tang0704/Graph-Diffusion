...
for ii=1:1
    ...
    switch priorOpts.priortype
        ...
        otherwise
            ...
            if SHOW
                ...
                if SHOW
                    %                     figure('name','backseeds'); imshow(label_im_neg);
    ...
    %% perform learning-based les / select seeds!  �����ʼ����ֵ�����¶����?
    ...
    compare_priors = {'back','color','center','our'};
    
    for num_prior = 1:numel(compare_priors)
        
        switch compare_priors{num_prior}
            case 'back'
                prior_cue = prior_source;
            case 'color'
                prior_cue = cue_by_color;
            case 'center'
                prior_cue = cueImgCnt;
            case 'our'
                prior_cue = prior_source_with_cue;
        ...
        [sp_saliency, pos_label_inds_choosen] = solve_submodular_naive(...
            aff_mat, seed_num, pos_label_inds, neg_label_inds, prior_cue, ...
            submodularOpts,input_im, superpixels,show_para,SHOW,saldir,imname); 
        
        
        %% assign the saliency value to each pixel
        [label_im] = draw_seed(input_im, superpixels,  pos_label_inds_choosen, [255,0,0]);
        soutname=[saldir imnames(ii).name(1:end-4) '_' compare_priors{num_prior} '_seed.png'];
        ...
        outname=[saldir imnames(ii).name(1:end-4) '_' compare_priors{num_prior},'.bmp'];
