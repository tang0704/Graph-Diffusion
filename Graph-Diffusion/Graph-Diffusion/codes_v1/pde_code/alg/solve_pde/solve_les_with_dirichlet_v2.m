function [ranks] = solve_les_with_dirichlet_v2(aff_mat, pos_label_inds, pos_label_ranks,...
...
ground_cond = ground_conductance_coeff_foreground.*ones(num_vtx, 1); % tune this parameters
...
alpha = getoptions(opts, 'alpha', 0.99);
opt.Walpha = getoptions(opts, 'Walpha',0);  %0 : (D - W +alpha*I)F = alpha*Y  1:  (D - 1/(1+alpha)*W)F = alpha/(1+alpha)*D*Y. Both are same!!!

% Compute Laplace
D = diag(sum(aff_mat,2));

if opt.Walpha    
    lap_mat = D - 1/(1+alpha)*aff_mat;
    A = lap_mat(ulabel_inds, ulabel_inds);
    b = - lap_mat(ulabel_inds, label_inds)*label_ranks;    
    b = b + alpha/(1+alpha)*D(ulabel_inds)*prior_source(ulabel_inds);
else    
    lap_mat = D - aff_mat;
